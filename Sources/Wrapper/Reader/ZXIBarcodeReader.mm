...
- (NSArray<ZXIMetadata*> *)readMetaImageView: (ImageView)imageView {
    std::vector<Metadata> results = ZXing::GetMetadata(imageView, [ZXIBarcodeReader DecodeHintsFromZXIOptions:self.hints]);
    NSMutableArray* zxiResults = [NSMutableArray array];
    for (auto result: results) {
        [zxiResults addObject:
         [[ZXIMetadata alloc] initWithRow:[[NSNumber alloc] initWithInt:result.rows] columns:[[NSNumber alloc] initWithInt:result.columns] ecl:[[NSNumber alloc] initWithInt:result.ecl]
...
- (NSArray<ZXIMetadata *> *)readMetaCGImage:(nonnull CGImageRef)image {
    CGColorSpaceRef colorSpace = CGColorSpaceCreateWithName(kCGColorSpaceGenericGray);
    CGFloat cols = CGImageGetWidth(image);
    CGFloat rows = CGImageGetHeight(image);
    NSMutableData *data = [NSMutableData dataWithLength: cols * rows];
    ...
    ImageView imageView = ImageView(
              ...
              static_cast<int>(rows),
              ImageFormat::Lum);
    return [self readMetaImageView:imageView];
}
